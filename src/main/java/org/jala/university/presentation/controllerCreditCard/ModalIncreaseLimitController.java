...
import org.jala.university.services.endpoint.services.IncreaseLimit;
...
public class ModalIncreaseLimitController {
    ...
    private void updateLimitOnBackend(double newLimit) {
        try {
            boolean success = newLimit != 0 || newLimit !=  Double.parseDouble(String.valueOf(LimitCreditCardService.findLimitCreditByCardNumber(CardsPageController.getCardClicked()).getLimit_credit()));
            if(IncreaseLimit.increaseLimitIfNeeded(String.valueOf(LimitCreditCardService.findLimitCreditByCardNumber(CardsPageController.getCardClicked()).getLimit_credit()))) {
